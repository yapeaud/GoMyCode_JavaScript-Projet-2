...
// Programme pour afficher les premiers termes de la série de Fibonacci

// Demander à l'utilisateur le nombre de termes
let x = parseInt(prompt("Entrez le nombre de termes:"));

// Vérifier que l'entrée est valide
// Vérifie si l'entrée n'est pas un nombre ou si elle est inférieure ou égale à 0
if (isNaN(x) || x <= 0) {
    // Affiche un message d'erreur si l'entrée est invalide
    console.log("Veuillez entrer un nombre valide supérieur à 0.");
} else {
    // Affiche un message indiquant le nombre de termes à afficher
    console.log(`Les ${x} premiers termes de la série de Fibonacci sont:`);

    // Initialisation des deux premiers termes de la série
    let nbr1 = 0, nbr2 = 1, sum;

    // Boucle pour générer et afficher les termes de la série de Fibonacci
    for (let i = 0; i < x; i++) {
        if (i <= 1) {
            // Les deux premiers termes de la série sont 0 et 1
            sum = i;
        } else {
            // Calcule le terme suivant comme la somme des deux précédents
            sum = nbr1 + nbr2;
            // Met à jour les valeurs pour les itérations suivantes
            nbr1 = nbr2;
            nbr2 = sum;
        }
        // Affiche le terme actuel de la série
        console.log(sum);
    }
}
